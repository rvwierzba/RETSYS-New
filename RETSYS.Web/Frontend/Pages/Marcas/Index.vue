  ...
  <div class="min-h-screen bg-slate-50 p-6 font-sans">
    ...
    <div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <div v-if="!Marcas || Marcas.length === 0" class="text-center py-12 border-2 border-dashed border-slate-100 rounded-xl">
        ...
        <div v-else class="overflow-x-auto">
          <table class="w-full text-left border-collapse">
            ...
            <tbody>
              <tr v-for="marca in Marcas" :key="marca.id || marca.Id" class="border-b border-slate-50 hover:bg-slate-50/60 transition text-sm">
                ...
                <td class="py-4 text-center">
                  <span :class="(marca.ativo ?? marca.Ativo) ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-50 text-slate-500'" class="px-2.5 py-1 rounded-full text-xs font-medium">
...
// CORRIGIDO: Prop em PascalCase para capturar o retorno exato do MarcasController
defineProps({
  Marcas: Array
})

// Estado reativo estruturado para a API do C# receber via [FromBody]
const form = reactive({
  Nome: '',
  Descricao: '',
  Ativo: true
