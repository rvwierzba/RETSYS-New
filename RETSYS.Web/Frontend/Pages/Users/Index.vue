    ...
    <div class="max-w-6xl mx-auto space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div v-if="equipe.length === 0" class="text-center py-12 border-2 border-dashed border-slate-100 rounded-xl text-slate-400 text-sm">
          ...
          <div v-else class="overflow-x-auto">
            <table class="w-full text-left text-sm border-collapse">
              ...
              <tbody>
                <tr v-for="user in equipe" :key="user.id" class="border-b border-slate-50 hover:bg-slate-50/50 transition">
                  <td class="py-4">
                  ...
                  <td class="py-4 text-center text-sm font-medium text-slate-700">
                    {{ user.filialLoja || 'Não Informada' }}
                  </td>
                  <td class="py-4 text-center">
                    <span :class="user.ativo ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'" class="px-2.5 py-0.5 rounded-full text-xs font-bold border">
                      {{ user.ativo ? 'Ativo' : 'Inativo' }}
                    </span>
                  </td>
                  <td class="py-4 text-center">
                    <button 
                      @click="alterarStatusUsuario(user.id)"
                      :class="user.ativo ? 'bg-slate-950 hover:bg-slate-800' : 'bg-teal-600 hover:bg-teal-700'"
...
<script setup>
using { reactive } from 'vue' // Brincadeira, JS limpo puro sem mandarim!
